...
// Import the interop module so [JSImport] functions can resolve
...
// Cached arrays to avoid per-frame GC pressure (reused across frames)
bool[,]? _cachedBrailleGrid = null;
double[,]? _cachedDotAlt = null;
int _cachedDotW = 0, _cachedDotH = 0;
var _cachedProjectedPois = new List<(int cx, int cy, double distToCenter, string name, double z, int poiIdx)>();
var _cachedOccupiedRows = new Dictionary<int, List<(int start, int end)>>();

// Check if a resize was queued before .NET started (from main.js initial postMessage)
var initialResize = WasmPresentationAdapter.PollResize();
int initCols = 80, initRows = 24;
if (!string.IsNullOrEmpty(initialResize))
{
    var parts = initialResize.Split(',');
...
var adapter = new WasmPresentationAdapter(initCols, initRows);
    ...
    List<(string name, int cx, int cy, int poiIndex)> poiScreenOut)
{
    ...
    if (_cachedBrailleGrid == null || _cachedDotW != dotW || _cachedDotH != dotH)
    {
        _cachedBrailleGrid = new bool[dotW, dotH];
        _cachedDotAlt = new double[dotW, dotH];
        _cachedDotW = dotW;
        _cachedDotH = dotH;
    }
    else
    {
        Array.Clear(_cachedBrailleGrid);
        Array.Clear(_cachedDotAlt);
    }
    var brailleGrid = _cachedBrailleGrid;
    var dotAlt = _cachedDotAlt;
    ...
    foreach (var (a, b, level) in contourSegments)
    {
        ...
        // Back-face cull: both endpoints behind sphere
    ...
    // Render braille to surface with altitude-based color
    for (int cy = 0; cy < cellH; cy++)
    {
        for (int cx = 0; cx < cellW; cx++)
        {
            ...
            // Inline braille dot checks — avoids closure allocation
            ...
            if (pattern != 0)
            {
                ...
                char brailleChar = (char)(0x2800 + pattern);
                surface.WriteChar(cx, cy, brailleChar, foreground: color);
            }
        }
    }
    ...
    for (int pi = 0; pi < pois.Count; pi++)
    {
        ...
        _cachedProjectedPois.Add((cellX, cellY, distToCenter, poi.Name, z, pi));
        poiScreenOut.Add((poi.Name, cellX, cellY, pi));
    }

    // Sort by distance to viewport center (closest first = highest priority)
    _cachedProjectedPois.Sort((a, b) => a.distToCenter.CompareTo(b.distToCenter));

    // Track occupied cell ranges for label occlusion
    _cachedOccupiedRows.Clear();

    bool IsOccluded(int row, int colStart, int colEnd)
    {
        if (!_cachedOccupiedRows.TryGetValue(row, out var ranges)) return false;
    ...
    {
        if (!_cachedOccupiedRows.TryGetValue(row, out var ranges))
        {
            ranges = [];
            _cachedOccupiedRows[row] = ranges;
        }
        ranges.Add((colStart, colEnd));
    }

    foreach (var (cx, cy, dist, name, z, poiIdx) in _cachedProjectedPois)
    {
