...
/// <summary>
/// Interactive 3D globe with topographic contours, animated clouds, and selectable POIs.
/// Adapted from FlowDemo's SizzleCommand.
/// </summary>
internal class GlobeSelectionScenario : IWidgetScenario
{
    // Precomputed globe data (lazily initialized)
    private List<Vector3>? _vertices;
    private List<(Vector3 a, Vector3 b, double level)>? _contourSegments;
    private List<(Vector3 a, Vector3 b)>? _cloudSegments;
    private bool[]? _cloudShadow;
    private Dictionary<(int, int, int), List<int>>? _spatialGrid;
    private int _contourLevels;

    // Interaction state
    private Quaternion _rotQ = Quaternion.CreateFromAxisAngle(Vector3.UnitX, 0.35f);
    private double _zoom = 1.0;
    private int _lastSurfaceW = 80, _lastSurfaceH = 24;
    private readonly List<(string name, int cx, int cy, int poiIndex)> _poiScreenPositions = [];
    ...
    public Hex1bWidget Build(FlowStepContext ctx)
    {
        ...
        if (_selectedLocation is not null)
        {
            return ctx.VStack(v =>
            [
                v.Text($"  ✓ Selected: {_selectedLocation}"),
                v.Text(""),
                v.Text("  Press Ctrl+Q to return to scenario picker."),
            ]);
        }

        return ctx.VStack(vstack =>
        [
            vstack.Text("🌍 Right-click a location to select it. Drag to rotate, scroll to zoom."),
            vstack.Interactable(ic =>
                ic.Surface(s =>
                {
                    _lastSurfaceW = s.Width;
                    _lastSurfaceH = s.Height;

                    double weatherTime = (DateTime.UtcNow - _weatherStartTime).TotalSeconds;
                    var cloudDrift = Quaternion.CreateFromAxisAngle(Vector3.UnitY, (float)(weatherTime * 0.02));

                    return [s.Layer(surface => DrawGlobe(surface, _contourSegments!, _contourLevels,
                        _cloudSegments!, _cloudShadow!, _vertices!, _spatialGrid!, cloudDrift,
                        _rotQ, _zoom, Pois, _poiScreenPositions))];
                })
                .RedrawAfter(2000)
            )
            .WithInputBindings(bindings =>
            {
                bindings.Drag(MouseButton.Left).Action((startX, startY) =>
                {
                    ...
                    int dotW = _lastSurfaceW * 2, dotH = _lastSurfaceH * 4;
                    double radius = Math.Min(dotW, dotH) * 0.65 * _zoom;
                    ...
                    return DragHandler.Simple(
                        onMove: (dx, dy) =>
                        {
                            ...
                            _rotQ = Quaternion.Normalize(qYaw * qPitch * _rotQ);
                        },
                        onEnd: () => { }
                    );
                });

                bindings.Mouse(MouseButton.ScrollUp).Action(_ => _zoom = Math.Min(5.0, _zoom * 1.15));
                bindings.Mouse(MouseButton.ScrollDown).Action(_ => _zoom = Math.Max(0.3, _zoom * 0.87));
                ...
                bindings.Mouse(MouseButton.Right).Action(actionCtx =>
                {
                    ...
                    int bestIdx = -1;
                    for (int i = 0; i < _poiScreenPositions.Count; i++)
                    {
                        var sp = _poiScreenPositions[i];
                        double dx2 = sp.cx - mx;
                        double dy2 = sp.cy - my;
                    ...
                    var clicked = _poiScreenPositions[bestIdx];
                    _selectedLocation = Pois[clicked.poiIndex].Name;
                });
            })
            .Fill()
        ]);
    ...
    private void EnsureInitialized()
    {
        ...
        _weatherStartTime = DateTime.UtcNow;
