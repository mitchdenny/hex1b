...
internal class TerminalFailureScenario : IWidgetScenario
{
    ...
    private static readonly string s_command = string.Join(" && ",
        ...
        "sleep 0.5",
        "echo 'error: undefined reference to main'",
    ...
    public string Name => "Terminal (Failure)";
    public string Description => "Non-interactive terminal running a failing command";
        ...
        return ctx.Terminal(_handle!)
            .WhenNotRunning(args =>
            {
                ...
                return ctx.VStack(v =>
                {
                    ...
                    {
                        v.Text($"✗ Command failed (exit code: {exitCode})"),
                        v.Separator(),
                    };
