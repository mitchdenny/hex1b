...
public class ToggleSwitchNodeTests
{
    ...
    public async Task Integration_ToggleSwitch_RendersViaHex1bApp()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Manual"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ToggleSwitch_ArrowNavigates()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Off"), TimeSpan.FromSeconds(10))
            .Right()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_ToggleSwitch_MultipleNavigations()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Low"), TimeSpan.FromSeconds(10))
            .Right().Right()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_ToggleSwitch_CallbackTriggered()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Mode1"), TimeSpan.FromSeconds(10))
            .Right()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
