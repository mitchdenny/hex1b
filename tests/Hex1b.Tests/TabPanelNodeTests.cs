...
public class TabPanelNodeTests
{
    ...
    public async Task TabPanel_Renders_TabBar_And_Content()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Tab One") && s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "tab panel rendered")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task TabPanel_AltRight_SwitchesToNextTab()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "first tab")
            .Alt().Key(Hex1bKey.RightArrow)
            .WaitUntil(s => s.ContainsText("Content Two"), TimeSpan.FromSeconds(5), "switched to second tab")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_AltLeft_SwitchesToPreviousTab()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "switched back to first tab")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_InVStack_FirstPosition_TabsOnTop()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("TopTab"), TimeSpan.FromSeconds(5), "tab panel rendered")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_WithTabsOnBottom_TabsRenderAtBottom()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("BottomTab"), TimeSpan.FromSeconds(5), "tab panel rendered")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_OnSelectionChanged_FiresWhenTabSwitches()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "first tab")
            .Alt().Key(Hex1bKey.RightArrow)
            .WaitUntil(s => s.ContainsText("Content Two"), TimeSpan.FromSeconds(5), "switched")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        await runTask;
    ...
    public async Task TabPanel_AltRight_OnLastTab_WrapsToFirst()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "first tab")
            .Alt().Key(Hex1bKey.RightArrow)
            .WaitUntil(s => s.ContainsText("Content Two"), TimeSpan.FromSeconds(5), "second tab")
            .Alt().Key(Hex1bKey.RightArrow)
            .WaitUntil(s => s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "wrapped to first")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_SelectorDropdown_AppearsNearTabBar()
    {
        ...
        var initialSnapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Document1.cs") && s.ContainsText("▼"), TimeSpan.FromSeconds(5), "tab panel with selector rendered")
            .Capture("initial")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_DynamicallyAddingTab_UpdatesContent()
    {
        ...
        var snapshot1 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("No documents"), TimeSpan.FromSeconds(5), "empty state")
            .Capture("initial")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        ...
        var snapshot2 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content of File1"), TimeSpan.FromSeconds(5), "first file content")
            .Capture("afterAdd1")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        ...
        var snapshot3 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content of File2"), TimeSpan.FromSeconds(5), "second file content")
            .Capture("afterAdd2")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_TreeDoubleClick_UpdatesContent_WithoutExplicitInvalidate()
    {
        ...
        var snapshot1 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("File1.cs") && s.ContainsText("No documents"), TimeSpan.FromSeconds(5), "initial")
            .Capture("initial")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        Assert.True(snapshot1.ContainsText("No documents"), "Should show empty state initially");
        ...
        var snapshot4 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content of File 1"), TimeSpan.FromSeconds(5), "file1 content")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_SwitchingTabs_UpdatesContent()
    {
        ...
        var snapshot1 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Content One"), TimeSpan.FromSeconds(5), "tab1 content")
            .Capture("initial")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        ...
        var snapshot2 = await new Hex1bTerminalInputSequenceBuilder()
            .Alt().Key(Hex1bKey.RightArrow)
            .WaitUntil(s => s.ContainsText("Content Two"), TimeSpan.FromSeconds(5), "tab2 content")
            .Capture("afterSwitch")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_ManyTabs_NewlySelectedTab_ScrollsIntoView()
    {
        ...
        var snapshot1 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Tab1"), TimeSpan.FromSeconds(5), "initial render")
            .Capture("initial")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_MouseWheelOnTabBar_ScrollsTabs()
    {
        ...
        var snapshot1 = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("FirstTab"), TimeSpan.FromSeconds(5), "initial render")
            .Capture("initial")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task TabPanel_AddingNewTab_ScrollsToShowIt()
    {
        ...
        var snapshot2 = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("NewTab"), TimeSpan.FromSeconds(5), "new tab added")
            .Capture("afterAdd")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
