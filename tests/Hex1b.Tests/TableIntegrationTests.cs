...
public class TableIntegrationTests
{
    ...
    public async Task Table_FocusedRow_MovingDown_BackgroundFollowsFocus()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .Key(Hex1bKey.DownArrow)
            .WaitUntil(s => s.ContainsText("> Employee 3"), TimeSpan.FromSeconds(5), "focus on row 3")
            .Wait(100)
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Table_SelectionColumn_HeaderCheckboxDeselectsAllRows()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("▣") && s.ContainsText("Employee 1"),
    ...
    public async Task Table_ScrollToBottom_FocusBackgroundStillVisible()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("Employee 30"), TimeSpan.FromSeconds(5), "scrolled to bottom")
            .Wait(100)
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Table_HomeAndEnd_FocusNavigatesCorrectly()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Employee 1"), TimeSpan.FromSeconds(5), "table rendered")
            .Key(Hex1bKey.End)
            .WaitUntil(s => s.ContainsText("> Employee 20"), TimeSpan.FromSeconds(5), "at end")
            .Wait(100)
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .Key(Hex1bKey.Home)
            .WaitUntil(s => s.ContainsText("> Employee 1"), TimeSpan.FromSeconds(5), "back to home")
            .Wait(100)
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
