...
public class TerminalAnsiRenderingTests
{
    ...
    public async Task RenderFullSnapshot_ProducesAnsi()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Ready"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task RenderWithColors_ProducesCorrectColorCodes()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Colored"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task RenderWithClearScreen_IncludesClearAndHome()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Clear"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task RenderWithTextAttributes_IncludesSgrCodes()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Bold Button"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Capture_IncludesAnsiFile()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Capture"), TimeSpan.FromSeconds(10))
