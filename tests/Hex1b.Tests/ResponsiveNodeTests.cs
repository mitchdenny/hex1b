...
public class ResponsiveNodeTests
{
    ...
    public async Task Render_OnlyRendersActiveChild()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Visible"), TimeSpan.FromSeconds(5), "Visible text")
            .Capture("final")
            .Build()
    ...
    public async Task NestedResponsive_WorksCorrectly()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Inner"), TimeSpan.FromSeconds(5), "Inner text")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Responsive_WithOtherwiseFallback_ShowsFallback()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Fallback"), TimeSpan.FromSeconds(5), "Fallback text")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
