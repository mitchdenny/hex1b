...
public class DrawerNodeTests
{
    ...
    public async Task Drawer_InHStack_FirstChild_DirectionIsRight()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("»"), TimeSpan.FromSeconds(5), "collapsed drawer")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Drawer_InHStack_LastChild_DirectionIsLeft()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("«") && s.ContainsText("Main Content"), TimeSpan.FromSeconds(5), "right drawer collapsed")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_InVStack_FirstChild_DirectionIsDown()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("▼ Top") && s.ContainsText("Main Content"), TimeSpan.FromSeconds(5), "top drawer")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_InVStack_LastChild_DirectionIsUp()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("▲ Bottom") && s.ContainsText("Main Content"), TimeSpan.FromSeconds(5), "bottom drawer")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_Collapsed_ShowsCollapsedContent()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("[COLLAPSED]"), TimeSpan.FromSeconds(5), "collapsed content")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_Collapsed_NoContent_IsInvisible()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("MainContent"), TimeSpan.FromSeconds(5), "main content")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_ExpandedInline_ShowsExpandedContent()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("[EXPANDED]"), TimeSpan.FromSeconds(5), "expanded content")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_ExpandedInline_PushesMainContent()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("ExpandedPane") && s.ContainsText("MainContent"), TimeSpan.FromSeconds(5), "both visible")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_Overlay_Collapsed_IsFocusable()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("▼ Console"), TimeSpan.FromSeconds(5), "drawer visible")
    ...
    public async Task Drawer_Overlay_EnterKey_OpensPopup()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("POPUP_CONTENT"), TimeSpan.FromSeconds(5), "popup opened")
            .Capture("popup")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_Overlay_SpaceKey_OpensPopup()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("POPUP_SPACE"), TimeSpan.FromSeconds(5), "popup opened")
            .Capture("popup")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Drawer_OnExpanded_CallbackFires()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("▼ Open"), TimeSpan.FromSeconds(5), "drawer visible")
            .Tab()
            .Enter()
            .WaitUntil(s => s.ContainsText("Expanded"), TimeSpan.FromSeconds(5), "expanded")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        await runTask;
    ...
    public async Task Drawer_OnCollapsed_CallbackFires()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .Tab()
            .Enter()  // Open
            .WaitUntil(s => s.ContainsText("Close"), TimeSpan.FromSeconds(5), "popup opened")
