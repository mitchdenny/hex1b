...
public class ListNodeTests
{
    ...
    public async Task Render_SingleItem_ShowsItem()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Only Item"), TimeSpan.FromSeconds(10), "item visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_SelectedItem_HasSelectedIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("> Item 1"), TimeSpan.FromSeconds(10), "selected indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_UnselectedItems_HaveUnselectedIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("  Item 2"), TimeSpan.FromSeconds(10), "unselected indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_FocusedAndSelected_HasColorCodes()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Item 1"), TimeSpan.FromSeconds(10), "item visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_NotFocused_SelectedItemHasIndicatorOnly()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("> Item 1"), TimeSpan.FromSeconds(10), "indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_WithOffset_RendersAtCorrectPosition()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Test Item"), TimeSpan.FromSeconds(10), "item visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_WithCustomIndicator_UsesCustomIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("► Item 1"), TimeSpan.FromSeconds(10), "custom indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_WithCustomUnselectedIndicator_UsesCustomIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("- Item 2"), TimeSpan.FromSeconds(10), "custom unselected indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_RetroTheme_UsesTriangleIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("► Item 1"), TimeSpan.FromSeconds(10), "high contrast indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_NarrowTerminal_TruncatesItems()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Very"), TimeSpan.FromSeconds(10), "truncated content visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_MinimalWidth_StillRendersIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText(">"), TimeSpan.FromSeconds(10), "indicator visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListInApp_RendersCorrectly()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Option C"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListWithSelection_RendersIndicator()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("First"), TimeSpan.FromSeconds(10))
            .Down()
            .WaitUntil(s => s.ContainsText("> Second"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListReceivesFocus_HandlesInput()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Item 1"), TimeSpan.FromSeconds(10))
            .Down()
            .WaitUntil(s => s.ContainsText("> Item 2"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListActivation_InvokesCallback()
    {
        ...
        // Simulate Enter key
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Action 1"), TimeSpan.FromSeconds(10))
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_ListInVStack_RendersWithOtherElements()
    {
        ...
        // Capture snapshot BEFORE exiting the app
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Select an option:"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListWithTheme_AppliesTheme()
    {
        ...
        // Capture snapshot BEFORE exiting the app
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Themed Item"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListNavigationMultipleSteps_UpdatesSelection()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .Down()
            .Down()
            .WaitUntil(s => s.ContainsText("> Third"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ListInsideBorderWithOtherWidgets_RendersProperly()
    {
        ...
        // Capture snapshot BEFORE exiting the app
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Welcome"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_ConstrainedListWithKeyboardNavigation_RevealsItems()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Apple"), TimeSpan.FromSeconds(10))
            .Down().Down().Down() // Move to Date (index 3)
            .WaitUntil(s => s.ContainsText("> Date"), TimeSpan.FromSeconds(10), "Date to be selected")
    ...
    public async Task Integration_ConstrainedListWithMouseWheel_NavigatesCorrectly()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("Date"), TimeSpan.FromSeconds(10), "Date to appear after scrolling")
    ...
    public async Task Integration_LongList_KeyboardNavigationScrollsCorrectly()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Item 01"), TimeSpan.FromSeconds(10))
            .Down().Down().Down().Down().Down() // Items 2-6
            .Down().Down().Down().Down().Down() // Items 7-11
            .Down().Down().Down().Down() // Items 12-15
            .WaitUntil(s => s.ContainsText("> Item 15"), TimeSpan.FromSeconds(10))
            .Capture("at_item_15")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_LongList_MouseWheelScrollsThroughEntireList()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("> Item 20"), TimeSpan.FromSeconds(10))
            .Capture("at_end")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_MouseClickAfterScrolling_SelectsCorrectItem()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.ContainsText("> Item"), TimeSpan.FromSeconds(10), "An item should be selected with indicator")
            .Capture("after_click")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
