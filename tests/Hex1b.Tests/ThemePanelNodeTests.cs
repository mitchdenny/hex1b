...
public class ThemePanelNodeTests
{
    ...
    public async Task Render_RendersChildContent()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("ThemePanel Content"), TimeSpan.FromSeconds(5), "ThemePanel Content to appear")
            .Capture("final")
            .Build()
    ...
    public async Task Render_AppliesThemeMutation()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Test"), TimeSpan.FromSeconds(5), "Test button to appear")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_RestoresOriginalTheme()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Test"), TimeSpan.FromSeconds(5), "Test text to appear")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_WithNullMutator_RendersNormally()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("No Mutator"), TimeSpan.FromSeconds(5), "No Mutator text to appear")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_CachedTheme_IsUsed()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Cached"), TimeSpan.FromSeconds(5), "Cached button to appear")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_NestedThemePanels_ApplyThemesCorrectly()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Inner") && s.ContainsText("Outer"), TimeSpan.FromSeconds(5), "Inner and Outer buttons to appear")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
        Assert.True(snapshot.HasBackgroundColor(Hex1bColor.FromRgb(255, 0, 0)),
