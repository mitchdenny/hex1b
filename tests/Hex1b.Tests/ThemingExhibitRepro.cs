...
public class ThemingExhibitRepro
{
    ...
    public async Task TextBox_InSplitterRightPane_ShouldNotShowCursor_WhenListHasFocus()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Sample text"), TimeSpan.FromSeconds(10), "Wait for initial render")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task TextBox_WhenNotFocused_ShouldNotRenderCursorColors()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Item 1") && s.ContainsText("test text"), TimeSpan.FromSeconds(10),
    ...
    public async Task TextBox_WhenFocused_ShouldRenderCursorColors()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("test"), TimeSpan.FromSeconds(10), "TextBox content to appear")
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
