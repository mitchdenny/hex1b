...
public class EditorKeybindingDispatchTests
{
    ...
    public async Task LeftArrow_MovesCursorLeft()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.SearchPattern(cursorOnC).HasMatches,
                TimeSpan.FromSeconds(10), "cursor on C after two Rights")
            .Left()
            .WaitUntil(s => s.SearchPattern(cursorOnB).HasMatches,
                TimeSpan.FromSeconds(10), "cursor on B after Left")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task RightArrow_MovesCursorRight()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .Right()
            .WaitUntil(s => s.SearchPattern(cursorOnB).HasMatches,
                TimeSpan.FromSeconds(10), "cursor on B after Right")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task UpArrow_MovesCursorToPreviousLine()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .Down()
            .WaitUntil(s => s.SearchPattern(cursorOnB).HasMatches,
                TimeSpan.FromSeconds(10), "cursor on B after Down")
            .Up()
            .WaitUntil(s => s.SearchPattern(cursorOnA).HasMatches,
                TimeSpan.FromSeconds(10), "cursor back to A after Up")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task DownArrow_MovesCursorToNextLine()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.SearchPattern(cursorOnLine1).HasMatches,
                TimeSpan.FromSeconds(10), "cursor on line 1 after Down")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task HomeKey_MovesCursorToLineStart()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.SearchPattern(cursorAtCol0).HasMatches,
                TimeSpan.FromSeconds(10), "cursor at col 0 after Home")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task EndKey_MovesCursorToLineEnd()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.SearchPattern(cursorAtCol5).HasMatches,
                TimeSpan.FromSeconds(10), "cursor at end of line after End")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task CtrlHome_MovesCursorToDocumentStart()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.SearchPattern(cursorAtDocEnd).HasMatches,
                TimeSpan.FromSeconds(10), "cursor at document end")
            .Ctrl().Home()
            .WaitUntil(s => s.SearchPattern(cursorAtOrigin).HasMatches,
                TimeSpan.FromSeconds(10), "cursor at (0,0) after Ctrl+Home")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task PageDown_MovesCursorDownByViewportLines()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(_ => state.Document.OffsetToPosition(state.Cursor.Position).Line > initialLine,
                TimeSpan.FromSeconds(10), "cursor moved down after PageDown")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task PageUp_MovesCursorUpByViewportLines()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(_ => state.Document.OffsetToPosition(state.Cursor.Position).Line >= 5,
                TimeSpan.FromSeconds(10), "cursor moved after first PageDown")
            .PageDown()
            .WaitUntil(_ => state.Document.OffsetToPosition(state.Cursor.Position).Line >= 15,
                TimeSpan.FromSeconds(10), "cursor moved after second PageDown")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            }, TimeSpan.FromSeconds(10), "cursor moved up after PageUp")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task ShiftRight_ExtendsSelection()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(_ => state.Cursor.HasSelection && state.Cursor.Position.Value == 1,
                TimeSpan.FromSeconds(10), "selection extended right")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task CtrlBackspace_DeletesPreviousWord()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(_ => !state.Document.GetText().Contains("world", StringComparison.Ordinal),
                TimeSpan.FromSeconds(10), "previous word deleted")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task CtrlDelete_DeletesNextWord()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(_ => !state.Document.GetText().Contains("hello", StringComparison.Ordinal),
                TimeSpan.FromSeconds(10), "next word deleted")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task CtrlShiftK_DeletesCurrentLine()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(_ => !state.Document.GetText().Contains("AAA", StringComparison.Ordinal),
                TimeSpan.FromSeconds(10), "current line deleted")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task AnyCharacter_InsertsText()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .WaitUntil(s => s.SearchPattern(xAtCol0).HasMatches,
                TimeSpan.FromSeconds(10), "x typed with text colors")
            .Build()
            .ApplyAsync(terminal, TestContext.Current.CancellationToken);
