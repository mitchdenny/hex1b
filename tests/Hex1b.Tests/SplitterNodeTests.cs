...
public class SplitterNodeTests
{
    ...
    public async Task Render_ShowsDivider()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("│"),
                TimeSpan.FromSeconds(10), "divider character")
            .Capture("final")
            .Build()
    ...
    public async Task Render_DividerSpansFullHeight()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("│") || s.ContainsText("←") || s.ContainsText("→"),
                TimeSpan.FromSeconds(10), "divider characters visible")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_Splitter_RendersCorrectly()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Right Content"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_SplitterWithVStackPanes_RendersCorrectly()
    {
        ...
        // Capture snapshot BEFORE exiting
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Right 2"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_SplitterWithButtons_HandlesFocus()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Left"), TimeSpan.FromSeconds(10))
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_SplitterResize_ArrowKeysWork()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Left"), TimeSpan.FromSeconds(10))
            .Left().Left()
            .WaitUntil(s => s.ContainsText("Left"), TimeSpan.FromSeconds(10)) // Verify app still renders after resize
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_SplitterWithTextBox_HandlesTyping()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            ...
            .Type("Hello Splitter")
            .WaitUntil(s => s.ContainsText("Hello Splitter"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_SplitterInsideBorder_RendersCorrectly()
    {
        ...
        // Capture snapshot BEFORE exiting
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Split View"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_Vertical_WhenFocused_ShowsThumbIndicators()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("⠶"),
                TimeSpan.FromSeconds(10), "braille thumb on focused vertical divider")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_VSplitter_RendersCorrectly()
    {
        ...
        // Capture snapshot BEFORE exiting
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Bottom Content"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_VSplitterWithVStackPanes_RendersCorrectly()
    {
        ...
        // Capture snapshot BEFORE exiting
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Bottom 2"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_VSplitterWithButtons_HandlesFocus()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Top"), TimeSpan.FromSeconds(10))
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_VSplitterInsideBorder_RendersCorrectly()
    {
        ...
        // Capture snapshot BEFORE exiting
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Vertical Split"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_NestedSplitters_HorizontalInsideVertical()
    {
        ...
        // Capture snapshot BEFORE exiting
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Bottom"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_TabFromListInVStackInsideSplitter_MovesFocusToNextPane()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Right Button"), TimeSpan.FromSeconds(10))
            .Tab().Tab().Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_ShiftTabFromButtonInsideSplitter_MovesFocusToPreviousPane()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Left Button"), TimeSpan.FromSeconds(10))
    ...
    public async Task Integration_NestedSplitters_InnerSplitterDoesNotOverrideFocus()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Outer Left"), TimeSpan.FromSeconds(10))
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_TripleNestedSplitters_OnlyRootSetsFocus()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Level 1"), TimeSpan.FromSeconds(10))
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
    ...
    public async Task Integration_NestedSplitters_ContentDoesNotOverflowIntoBelowPane()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Bottom Pane"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_HorizontalSplitter_ContentDoesNotOverflowIntoRightPane()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Right Pane"), TimeSpan.FromSeconds(10))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_Vertical_ClipsTopPaneContent()
    {
        ...
        node.Render(context);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Line 1") && s.ContainsText("Bottom"), TimeSpan.FromSeconds(10), "splitter content to render")
            .Build()
            .ApplyAsync(terminal);
    ...
    public async Task Integration_NestedSplitters_ResizingInnerDoesNotCauseOverflow()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("BOTTOM_PANE_MARKER"), TimeSpan.FromSeconds(10), "initial render")
            .Tab()  // Move to inner splitter
            .Left().Left().Left().Left().Left()  // Resize left significantly (make left pane very narrow)
            .WaitUntil(s => s.ContainsText("BOTTOM_PANE_MARKER"), TimeSpan.FromSeconds(10), "resize to complete")
    ...
    public async Task Integration_NestedSplitters_WrappingTextDoesNotOverflowWhenDraggedExtreme()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Bottom Pane"), TimeSpan.FromSeconds(10), "initial render")
            .Tab()  // Focus VSplitter
            .Tab()  // Focus inner horizontal Splitter
            // Resize to extreme RIGHT (20+ right arrows - making right pane very narrow)
            .Right().Right().Right().Right().Right().Right().Right().Right().Right().Right()
            .Right().Right().Right().Right().Right().Right().Right().Right().Right().Right()
            .WaitUntil(s => s.ContainsText("Bottom Pane"), TimeSpan.FromSeconds(10), "resize to complete")
