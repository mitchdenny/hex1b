...
public class HStackNodeTests
{
    ...
    public async Task Render_RendersAllChildren()
    {
        ...
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Left") && s.ContainsText("Right"), TimeSpan.FromSeconds(5), "Left and Right text")
            .Capture("final")
            .Build()
    ...
    public async Task Render_ChildrenAppearOnSameLine()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("AAA") && s.ContainsText("BBB") && s.ContainsText("CCC"), TimeSpan.FromSeconds(5), "AAA BBB CCC on same line")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Render_InNarrowTerminal_TextWraps()
    {
        ...
        node.Render(context);
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("AAAA"), TimeSpan.FromSeconds(5), "AAAA text")
            .Capture("final")
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_HStack_TabNavigatesThroughFocusables()
    {
        ...
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Btn1"), TimeSpan.FromSeconds(5))
            .Tab()
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
    ...
    public async Task Integration_HStack_InNarrowTerminal_StillWorks()
    {
        ...
        // Capture snapshot BEFORE exiting the app
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("C"), TimeSpan.FromSeconds(5))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_HStack_WithVStack_NestedLayouts()
    {
        ...
        // Capture snapshot BEFORE exiting the app
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Right Bottom"), TimeSpan.FromSeconds(5))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_HStack_WithMixedContent()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Label:"), TimeSpan.FromSeconds(5))
            .Type("Hi")
            .Tab()
            .Enter()
            .Capture("final")
    ...
    public async Task Integration_HStack_LongContentInNarrowTerminal_Wraps()
    {
        ...
        // Capture snapshot BEFORE exiting the app
        var snapshot = await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("VeryLongWord"), TimeSpan.FromSeconds(5))
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
            .Build()
            .ApplyWithCaptureAsync(terminal, TestContext.Current.CancellationToken);
    ...
    public async Task Integration_TabFromSingleFocusableVStackInHStack_BubblesUpToHStack()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("Add"), TimeSpan.FromSeconds(5))
            .Tab()
            .Enter()
            .Capture("final")
            .Ctrl().Key(Hex1bKey.C)
    ...
    public async Task Integration_NestedVStackWithMultipleFocusables_TabEscapesAtBoundary()
    {
        ...
        var runTask = app.RunAsync(TestContext.Current.CancellationToken);
        await new Hex1bTerminalInputSequenceBuilder()
            .WaitUntil(s => s.ContainsText("List"), TimeSpan.FromSeconds(5))
