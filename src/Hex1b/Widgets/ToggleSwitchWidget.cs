...
public sealed record ToggleSwitchWidget(IReadOnlyList<string> Options, int SelectedIndex = 0) : Hex1bWidget
{
    ...
    public static readonly ActionId PreviousOptionActionId = new($"{nameof(ToggleSwitchWidget)}.{nameof(PreviousOptionActionId)}");

    /// <summary>Action ID for moving to the next option.</summary>
    public static readonly ActionId NextOptionActionId = new($"{nameof(ToggleSwitchWidget)}.{nameof(NextOptionActionId)}");

    /// <summary>Action ID for selecting an option via mouse click.</summary>
    public static readonly ActionId SelectOptionActionId = new($"{nameof(ToggleSwitchWidget)}.{nameof(SelectOptionActionId)}");
