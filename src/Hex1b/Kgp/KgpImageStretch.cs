...
public enum KgpImageStretch
{
    ...
    /// (~10 px/column, ~20 px/row). If the image is larger than the
    /// allocated area it will be clipped.
    /// </summary>
    None,

    /// <summary>
    /// The image is scaled to fit within the allocated display area while
    /// preserving the aspect ratio, maximizing one dimension. The resulting
    /// size may be smaller than the available space in one dimension.
    /// Wrap in <see cref="Widgets.AlignWidget"/> to control positioning.
    /// </summary>
    Fit,
    ...
    /// <summary>
    /// The image is stretched to fill the allocated cell dimensions.
