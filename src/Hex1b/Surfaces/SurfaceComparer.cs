...
public static class SurfaceComparer
{
    ...
    /// Uses <see cref="KgpVisibility"/> to split the image into multiple fragments
        ...
        int anchorX, int anchorY, KgpCellData data, Surface surface)
    {
        ...
        for (var y = anchorY; y < regionBottom; y++)
        {
            ...
            for (var x = anchorX; x <= regionRight; x++)
            {
                ...
                else if (runStartX.HasValue)
                {
                    // End of an occluded run — apply it
                    visibility.ApplyOcclusion(
                        new Layout.Rect(runStartX.Value, y, x - runStartX.Value, 1), metrics);
                    runStartX = null;
        ...
        // If still fully visible, emit the original data unchanged
        ...
        // Generate clipped placements for each visible fragment
        var placements = visibility.GeneratePlacements(metrics);
        foreach (var (cx, cy, clipped) in placements)
        {
            result.Add((cx, cy, clipped));
        }
